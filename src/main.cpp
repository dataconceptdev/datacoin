...
#if defined(NDEBUG)
# error "Datadev cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xfc5790f70a40905fbf38872dbbec6e9c9b04ce7549b5d104ca2dc5cf64d5082f");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Datadev: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Datadev Signed Message:\n";
...
{
    // Datadev: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // Datadev: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Datadev: 3.5 days
static const int64 nTargetSpacing = 5 * 60; // Datadev: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Datadev: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Datadev: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x438cc2a50aa205c86283e3b71b2c015634f7ef2d93afcf64fcb262148bc37925");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04f57e8a06d4df5392c416dfeeda88ccdc9ddbb190e0d80850de66b92605c7b39dfa03803426d9b19b5b482a602bbcbc88b946562d03dbb08e36804f2b530820c5") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1617531930;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084646851;

        if (fTestNet)
        {
            block.nTime    = 1617531882;
            block.nNonce   = 386683212;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x90827e9d3507e4dc787ee459070c34c95b7367cc1136ca2084127acf60ca9371"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xba, 0xd3 }; // Datadev: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("DatadevMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("DatadevMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static DatadevMiner(CWallet *pwallet)
{
    printf("DatadevMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("datadev-miner");
    ...
    try { loop {
        ...
        printf("Running DatadevMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("DatadevMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&DatadevMiner, pwallet));
}
